...
def kfold_split(ds_train, id_key='label', k=5, val_fold=0, seed=15):
    """
    ...
    The dataset is split in such a way that the sets of ``id_key``s in training
    and validation sets is mutually exclusive.
    """
    assert val_fold < k
    ...
    df_train = df_train[df_train[id_key].isin(train_labels)]
    df_val = df_val[df_val[id_key].isin(val_labels)]
